...
pub(crate) fn render_markdown_text_with_width_and_cwd(
    ...
    let parser = Parser::new_ext(&normalized_input, options).into_offset_iter();
    let mut w = Writer::new(&normalized_input, parser, width, cwd);
    w.run();
    w.text
}
    ...
    I: Iterator<Item = (Event<'a>, Range<usize>)>,
{
    ...
    fn push_image_alt_fallback(&mut self, alt_text: String) {
        ...
        let style = self.inline_styles.last().copied().unwrap_or_default();
        let span = Span::styled(alt_text, style);
        if self.in_table_cell() {
